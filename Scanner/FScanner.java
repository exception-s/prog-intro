...
public class FScanner implements AutoCloseable {
    ...
    public FScanner(InputStream source) {
        READER = new InputStreamReader(source, StandardCharsets.UTF_8);
    }
    public FScanner(File file) throws FileNotFoundException {
    ...
    void skipWhitespaces() throws IOException{    //test
        while (haveInput() && Character.isWhitespace(BUFFER[pointer]) && BUFFER[pointer] != LINE_SEP.charAt(0)) {
            pointer++;
            if (pointer >= read) {
                reset();
            }
